...
class TestSourceHandlerBase < Test::Unit::TestCase
  ...
  def test_output_path
    ...
    assert_equal('path.html', @temp.output_path(node, path, [:cnbase, ['.', :lang], :ext]))
    path = Webgen::Path.new('path.en.html')
    assert_equal('path.html', @temp.output_path(node, path, [:cnbase, ['.', :lang], :ext]))
    path = Webgen::Path.new('path.eo.html')
    assert_equal('path.eo.html', @temp.output_path(node, path, [:cnbase, ['.', :lang], :ext]))

    other = Webgen::Node.new(node, 'path.html')
    path = Webgen::Path.new('path.html')
    assert_equal('path.html', @temp.output_path(node, path, [:cnbase, ['.', :lang], :ext]))
    path = Webgen::Path.new('path.en.html')
    assert_equal('path.en.html', @temp.output_path(node, path, [:cnbase, ['.', :lang], :ext]))
  end
